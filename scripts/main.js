...
function updateBookmark(hikeDocID) {
    currentUser.get().then(userDoc => {
        ...
        if (bookmarks.includes(hikeDocID)) {
            document.getElementById('save-' + hikeDocID).innerText = 'bookmark_border';
            var current_bookmarks = userDoc.data().bookmarks
            toRemove = current_bookmarks.indexOf(hikeDocID)
            var updated_bookmarks = current_bookmarks
            updated_bookmarks.splice(toRemove, 1)
            currentUser.set({bookmarks: updated_bookmarks})
        } else {
            saveBookmark(hikeDocID)
        }
    })
}